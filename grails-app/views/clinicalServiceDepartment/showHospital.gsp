...
<head>
    ...
    <title>${healthServiceInstance.name} ${clinicalServiceDepartmentInstance.name}</title>
</head>
...
<div id="show-ward" class="content scaffold-show" role="main">
    <h1>${healthServiceInstance.name} - ${clinicalServiceDepartmentInstance.name}</h1>
    ...
    <div class="custom-body">
        ...
        <g:render template="/common/chart" model="${[args: [id: clinicalServiceDepartmentInstance.id, healthServiceId: healthServiceInstance.id], questions: questions]}"/>
        ...
        <table class="ratings" style="float: left">
            ...
            <tr class="odd">
                <g:set var="rating" value="${clinicalServiceDepartmentInstance.getNpsRatingForHealthService(healthServiceInstance)}"/>
                <td class="question">NPS Rating</td>
                <td class="score"><pf:stars value="${rating.rating}" count="${rating.count}"/></td>
                <td>${rating.count}</td>
            </tr>
            <g:each in="${healthServiceInstance.type.questions.findAll({it.type == QuestionType.RATING})}" var="question" status="i">
                <tr class="${i % 2 == 1 ? 'odd' : 'even'}">
                    <g:set var="rating" value="${clinicalServiceDepartmentInstance.getRatingForQuestionAndHealthService(question, healthServiceInstance)}" />
            ...
            <tr class="total">
                <g:set var="overallRating" value="${clinicalServiceDepartmentInstance.getRatingForHealthService(healthServiceInstance)}" />
        ...
        <g:render template="/common/comments" model="${[feedbacks: feedbacks, totalCount: feedbacks.totalCount, id: healthServiceInstance.id, params: [healthServiceId: healthServiceInstance.id]]}" />
    </div>
